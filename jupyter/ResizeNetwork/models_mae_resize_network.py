...
class ResizeBlock(nn.Module):
    def __init__(self, in_channels=32, out_channels=32,
                 kernel_size=7, target_height=224, target_width=224, slope=0.1):
        super().__init__()
...
class ResizeNetwork(nn.Module):
    def __init__(self, num_res_block, target_height, target_width):
        ...
        #(480, 270) -> (224, 224)
        self.layer3 = ResizeBlock(in_channels=3, out_channels=32,
                                     kernel_size=7, target_height=224, target_width=224)
...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3,
                 ...
                 mlp_ratio=4., norm_layer=nn.LayerNorm, norm_pix_loss=False, resize_network=None):
        super().__init__()

        # --------------------------------------------------------------------------
        # MAE encoder specifics
        self.custom_patch_size = 2
    ...
    def forward(self, imgs, mask_ratio=0.4, flag=False):
        imagenet_mean = np.array([0.485, 0.456, 0.406])
        imagenet_std = np.array([0.229, 0.224, 0.225])
        
        if flag:
            image = torch.einsum('nchw->nhwc', imgs).cpu()
            plt.rcParams['figure.figsize'] = [24, 24]
            plt.subplot(1, 2, 1)
            plt.imshow(torch.clip((image[0] * imagenet_std + imagenet_mean) * 255, 0, 255).int())
            """plt.subplot(1, 4, 2)
            plt.imshow(torch.clip((image[1] * imagenet_std + imagenet_mean) * 255, 0, 255).int())
            plt.subplot(1, 4, 3)
            plt.imshow(torch.clip((image[2] * imagenet_std + imagenet_mean) * 255, 0, 255).int())
            plt.subplot(1, 4, 4)
            plt.imshow(torch.clip((image[3] * imagenet_std + imagenet_mean) * 255, 0, 255).int())"""
            #plt.show()
        
        imgs = self.forward_resize(imgs)
        
        if flag:
            image = torch.einsum('nchw->nhwc', imgs).cpu().detach()
            plt.subplot(1, 2, 2)
            plt.imshow(torch.clip((image[0] * imagenet_std + imagenet_mean) * 255, 0, 255).int())
            """plt.subplot(1, 4, 2)
            plt.imshow(torch.clip((image[1] * imagenet_std + imagenet_mean) * 255, 0, 255).int())
