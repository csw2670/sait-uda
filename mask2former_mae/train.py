...
def miou(result_pred, result_gt):
...
def validation(unet_model, device, unet_train_dataloader):
	with torch.no_grad():
		unet_model.eval()
		final_result = []
		for images, masks in tqdm(unet_validation_dataloader):
			...
			for idx in range(len(outputs)):
				batch_results.append(miou(outputs[idx], masks[idx]))
...
def main(args):
	...
	validation(unet_model, args.device, unet_validation_dataloader)
