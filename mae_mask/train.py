...
def get_args_parser():
	...
	parser.add_argument('--pretrained_ckpt_dir', default='VitMae_fft_full_data_mask04_e200.pt', type=str,
                        help='validation dataset path')
	parser.add_argument('--pretraining_ckpt_dir', default='VitMae_fft_full_data_mask04_e400.pt', type=str,
                        help='validation dataset path')														
...
def pretrain_mae(args):
	...
	if args.save_pretrained_model == 'y':
		save_file_name = './VitMae_fft_full_data_mask04_e400.pt'
		if os.path.isfile(save_file_name):
			os.remove(save_file_name)
		torch.save(mae_model, './VitMae_fft_full_data_mask04_e400.pt')
...
def import_mae(args):
	if args.determine_pretrain == 'y':
		...
		ckpt_dir = args.pretrained_ckpt_dir
...
def main(args):
	...
	train(final_model, args.train_epochs, args.device, unet_train_dataloader, optimizer, criterion)
...
if __name__ == '__main__':
	sys.path.insert(0, '../jupyter/Normal')
